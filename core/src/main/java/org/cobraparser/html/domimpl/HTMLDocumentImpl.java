...
public class HTMLDocumentImpl extends NodeImpl implements HTMLDocument, DocumentView, DocumentStyle, EventTarget {
    ...
    private final org.cobraparser.html.js.Window window;
    ...
    public HTMLDocumentImpl(final UserAgentContext ucontext, final HtmlRendererContext rcontext, final WritableLineReader reader,
        ...
        this.document = this;
        // Get Window object
        org.cobraparser.html.js.Window window;
        if (rcontext != null) {
            window = JavaScriptEngine.get().getWindow(rcontext);
        }
        else {
            // Plain parsers may use Javascript too.
            window = JavaScriptEngine.get().createWindow(null, ucontext);
        }
    ...
    private final List<Object> onloadHandlers = new ArrayList<>();

    public Object getOnloadHandler() {
        return onloadHandler;
    }

    public void setOnloadHandler(final Object onloadHandler) {
    ...
    private void dispatchLoadEvent() {
        ...
        final Event domContentLoadedEvent = JavaScriptEngine.get().createEvent("DOMContentLoaded", getBody()); // TODO: What should be the target for this event?
    ...
    @HideFromJS
    public void addLoadHandler(final Object handler) {
        onloadHandlers.add(handler);
    }

    @HideFromJS
    public void removeLoadHandler(final Object handler) {
    ...
    public Event createEvent(final String type) {
        return JavaScriptEngine.get().createEvent(type, this);
    }
